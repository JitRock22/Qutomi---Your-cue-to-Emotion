...
const Signup = ({ setIsLoggedIn }) => {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      // No localStorage - just update auth state
      setIsLoggedIn(true);

      // Set flag to show skeleton on home page
      localStorage.setItem('showHomeSkeleton', 'true');
  ...
  >
    <motion.button
      ...
      disabled={googleLoading || loading}
      whileHover={{
        scale: (googleLoading || loading) ? 1 : 1.05,
        boxShadow: (googleLoading || loading) ? "none" : "0 10px 30px -10px rgba(6, 182, 212, 0.3)"
      }}
      whileTap={{ scale: (googleLoading || loading) ? 1 : 0.98 }}
      className="w-full py-3 px-4 bg-white/5 border border-white/10 text-white font-semibold rounded-xl transition-all duration-300 flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed relative overflow-hidden group"
    >
      {/* Blue to Cyan Gradient - Bridges Google Blue and Your Cyan */}
      <div className="absolute inset-0 bg-gradient-to-r from-[#3B82F6] to-[#06B6D4] opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-xl" />
